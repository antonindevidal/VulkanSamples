...
void Swapchain::cleanup(Device& device)
{
	...
	for (size_t i = 0; i < _swapChainImageViews.size(); i++) {
		vkDestroyImageView(device.getDevice(), _swapChainImageViews[i], nullptr);
	}
...
void Swapchain::transitionImageLayout(std::shared_ptr<Context> context, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
...
VkFormat Swapchain::findSupportedFormat(Device & device, const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
{
